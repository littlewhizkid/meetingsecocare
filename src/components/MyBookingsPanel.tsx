...
export const MyBookingsPanel = ({
  ...
  const filtered = normalizedFilter
    ? bookings.filter(
        (booking) =>
          booking.bookerName.toLowerCase().includes(normalizedFilter) ||
          booking.meetingTitle.toLowerCase().includes(normalizedFilter),
      )
    : bookings;
            ...
            >
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  Booked by {booking.bookerName} ({booking.userEmail})
                </p>
              </div>
